...
function tdd_generate_yourls( $post_id ) {
	$permalink  = get_permalink( $post_id );
	$yourls_api = '254f93140c';
	$req_url    = 'http://tdd.li/nk/yourls-api.php?signature=' . $yourls_api . '&action=shorturl&format=json&url=' . $permalink;

	// Access my Yourls API to get a short URL.
	$yourls_response = wp_remote_retrieve_body( wp_remote_get(  $req_url ) );

	if ( empty( $yourls_response ) )
		return false;

	$yourls_response = json_decode( $yourls_response );

	if ( !isset( $yourls_response->shorturl ) || empty( $yourls_response->shorturl ) )
		return false;

	update_post_meta( $post_id, 'yourls', esc_url_raw( $yourls_response->shorturl ) );
	return esc_url( $yourls_response->shorturl );
}

function tdd_shortlink_filter( $shortlink, $id, $context, $allow_slugs ) {

	if ( $context != 'post' )
		return false;

	// First check post meta
	$yourls = get_post_meta( $id, 'yourls' );

	// No dice, try to generate one.
	if ( empty( $yourls ) )
		$yourls = tdd_generate_yourls( $id );

	return $yourls;

}
add_filter('pre_get_shortlink', 'tdd_shortlink_filter', 10, 4);
